...
############################ fix-SA comparison/operators

from sqlalchemy import sql
sqltypes = sql.sqltypes

class _patchCompareMixin:
    def in_(self, *other):
        if len(other) == 0:
            return self.__eq__(None)
        elif len(other) == 1:
            o = other[0]
            if sql._is_literal(o) or isinstance( o, sql._CompareMixin):
        ...
        args = []
        for o in other:
            if not sql._is_literal(o):
                if not isinstance( o, sql._CompareMixin):
                    raise sql.exceptions.InvalidRequestError( "in() function accepts either non-selectable values, or a single selectable: "+repr(o) )
            else:
    ...
    def startswith(self, other):
        perc = isinstance(other,(str,unicode)) and '%' or sql.literal('%',type= sqltypes.String)
        return self._compare('LIKE', other + perc)
    def endswith(self, other):
        if isinstance(other,(str,unicode)): po = '%' + other
        else:
            po = sql.literal('%', type= sqltypes.String) + other
...
for k,v in _patchCompareMixin.__dict__.iteritems():
    if callable(v):
        setattr( sql._CompareMixin, k, v)

############################ eo fix-SA comparison/operators
